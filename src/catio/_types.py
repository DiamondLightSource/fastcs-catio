...
class AmsNetId:
    ...
    def from_bytes(cls, net_id: bytes) -> Self:
        ...
        :params net_id: the netid value expressed as 6 bytes

        :returns: the AmsNetId expressed as tuples of integers

        :raises ValueError: if the netid is not exactly 6 bytes long
        """
        assert len(net_id) == 6, ValueError("AMS NetID must be exactly 6 bytes long.")
    ...
    def from_string(cls, net_id: str) -> Self:
        ...
        :params net_id: the netid value expressed as a string of format x.x.x.x.x.x
        ...
        assert len(parts) == 6, ValueError(
            "AMS NetID must be exactly 6 dot-separated octets."
        )
        return cls((parts[0], parts[1], parts[2], parts[3]), (parts[4], parts[5]))

    def to_bytes6(self) -> bytes:
        """
