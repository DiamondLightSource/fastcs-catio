...
class CompositeTypeMember(BaseModel):
    ...
    bit_size: int | None = Field(default=None, description="Bit size if less than 8")
    bit_offset: int | None = Field(
        default=None, description="Bit offset within the byte/word"
    )
...
class TerminalType(BaseModel):
    ...
    data_types: dict[str, CompositeType] = Field(
        default_factory=dict, description="Custom data type definitions"
    )
...
class TerminalConfig(BaseModel):
    ...
    def to_yaml(self, path: Path) -> None:
        ...
        for _terminal_id, terminal_data in data.get("terminal_types", {}).items():
            ...
            if "data_types" in terminal_data and not terminal_data["data_types"]:
                del terminal_data["data_types"]
